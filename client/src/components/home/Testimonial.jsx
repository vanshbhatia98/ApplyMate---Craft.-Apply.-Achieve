...
const Testimonial = () => {
    ...
    const cardsData = [
     ...
     const CreateCard = ({ card }) => (
        <div className="p-5 rounded-2xl mx-3 bg-slate-900 border border-slate-800 shadow-lg shadow-black/20 hover:border-slate-700 hover:-translate-y-0.5 transition-all duration-300 w-72 shrink-0">
            <div className="flex gap-3">
                <img className="size-11 rounded-full object-cover ring-2 ring-slate-800 shadow" src={card.image} alt="User Image" />
                <div className="flex flex-col">
                    <div className="flex items-center gap-1">
                        <p className="font-medium text-slate-100">{card.name}</p>
                        <svg className="mt-0.5 fill-emerald-400" width="12" height="12" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
            ...
            <p className="text-sm py-4 text-slate-400 leading-relaxed">Radiant made undercutting all of our competitors an absolute breeze.</p>
        </div>
    );
  ...
  return (
    ...
    <div id='testimonials' className='flex flex-col items-center my-10 scroll-mt-12'>
        <div className="flex items-center gap-2 text-sm font-medium text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-5 py-1.5">
            <BookUserIcon className="size-4.5 stroke-emerald-400"/>
